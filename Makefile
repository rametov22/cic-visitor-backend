...
.PHONY: help dev prod up run down down-v build pull deploy logs shell dbshell \
        migrate makemigrations makemessages compilemessages collectstatic \
        createsuperuser test lint format dump restore flush-redis bash \
        bash-db bash-nginx

help:
	@echo "make dev up / make dev run   - development"
	@echo "make up                     - local production-like stack"
	@echo "make prod deploy            - pull and deploy on a server"
	@echo "make dev test/lint/format   - quality checks"
	@echo "make dump media / restore   - database and media migration"
